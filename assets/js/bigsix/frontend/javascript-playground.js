// ============================================================
//  javascript-playground.js — Single Responsibility: JS playground
...
//
//  EXPORTS:
//    initJsPlayground()  — call once on DOMContentLoaded
// ============================================================
...
const JS_EXAMPLES = {
   ...
   */
  function renderConsoleOutput(lines, error) {
   ...
   *
   * @param {string} key - Key from JS_EXAMPLES (e.g. 'variables')
   */
  function loadExample(key) {
    const ex = JS_EXAMPLES[key];
    if (!ex) return;
   ...
   */
  export function initJsPlayground() {
    ...
    document.getElementById('jsRunBtn').addEventListener('click', () => {
      ...
      const { lines, error } = executeCode(document.getElementById('jsInput').value);
      renderConsoleOutput(lines, error);
    });
  
    // Wire Clear button
    document.getElementById('jsClearBtn').addEventListener('click', clearConsole);
