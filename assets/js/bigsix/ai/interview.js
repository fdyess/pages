...
function buildInterviewPrompt(question) {
...
function buildScoreRowsHtml(scores) {
  const labels = {
...
function renderAnalysis(parsed) {
  ...
  html += `<div style="display:grid;gap:8px;margin-bottom:16px;">${buildScoreRowsHtml(parsed.scores || {})}</div>`;
...
export async function analyzeInterview() {
  ...
  callGemini(answer, buildInterviewPrompt(question))
    .then(({ text }) => parseJsonResponse(text))
    .then(parsed    => renderAnalysis(parsed))
    .catch(err => {
